...
class DecodingErrorLocalizedParserTests: XCTestCase {
    ...
    fileprivate static let requestURL: URL = URL(string: "https://example.com/vehicle_data?vin=1ABCDEFG&secret=FD432FDDS89")!

    // MARK: - Unit Testing
    let decoder = JSONDecoder()

    /// Tests `DecodingError.keyNotFound`.
    func testMissingKey() {
    ...
    /// Tests `DecodingError.typeMismatch`.
    ...
    /// Tests `DecodingError.valueNotFound`.
    func testUnexpectedNullValueOrValueNotFound() {
        ...
        XCTAssertThrowsError(try decoder.decode(Vehicle.self, from: nullValue), "") { error in
            let obaError: NSError = OBADecodingError(error as! DecodingError, url: Self.requestURL)! as NSError
            XCTAssertFalse(obaError.localizedDescription.isEmpty, "Localized description should not be empty.")
            XCTAssertTrue(obaError.localizedFailureReason!.contains("owner.contactInfo.phoneNumber"), "ValueNotFound localized should contain the path \"owner.contactInfo.phoneNumber\" somewhere in the string.")
            XCTAssertTrue(obaError.localizedFailureReason!.lowercased().contains("string"), "ValueNotFound localized should contain the expected type, \"String\", somewhere in the string.")
            XCTAssertEqual(obaError.userInfo[NSURLErrorKey] as! URL, Self.requestURL)
        }
    }
}
